...
export const process_json = (json: any, func: any) => {
  function prcs(x: any, n: any, pr: any) {
    let tp = typeof n[x];
    let v1 = `${pr}/${x}`;
    if (Array.isArray(n[x])) lst(n[x], v1); // @ts-check
    else if (tp == 'object') jsn(n[x], v1);
    else set_val_from_adress(v1, json, func(val_from_adress(v1, json)));
  }
  ...
  if (typeof json == 'object') jsn(json);
  else if (Array.isArray(json)) lst(json);
  return json;
};
